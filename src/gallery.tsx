...
export default function Gallery() {
  ...
  return (
    <div className="p-4">
      ...
      {items === undefined ? (
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4">
          {items.map((m) => {
            ...
            return (
              <figure key={media._id} className="group relative">
                <Wrapper
                ...
                <figcaption className="mt-1 text-sm">
                  <TitleField
                    media={media}
                    canEdit={user?._id === media.uploadedBy}
                  />
                  <div className="text-gray-600">
                    {media.uploaderName} -{" "}
                    {format(new Date(media._creationTime), "MMMM d, yyyy")}
                  </div>
                </figcaption>
              </figure>
            )
      ...
      {playing && (
        <VideoModal media={playing} onClose={() => setPlaying(null)} />
      )}
...
function TitleField({ media, canEdit }: { media: Media; canEdit: boolean }) {
  ...
  if (!canEdit) {
    return (
      <div className="truncate font-medium">{media.title ?? "Untitled"}</div>
    )
  }
  ...
  if (!editing) {
    return (
      <button
        ...
        className="-mx-1 w-full truncate rounded px-1 text-left font-medium hover:bg-gray-100"
  ...
  return (
    <input
      ...
      className="-mx-1 w-full rounded border px-1 font-medium"
    />
  )
}
