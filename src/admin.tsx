    ...
    <div className="flex flex-col gap-3">
        ...
        <ul className="flex flex-col gap-2">
          {bulletins.map((b) => (
            <BulletinRow
              key={b._id}
              initial={b.content}
              onSave={(content) => update({ token, id: b._id, content })}
      ...
      >
        <textarea
          ...
          rows={2}
          placeholder="Add a new bulletin…"
...
function BulletinRow({
  ...
  return (
    <li className="flex items-start gap-2 rounded border border-gray-200 p-2">
      <textarea
        value={value}
